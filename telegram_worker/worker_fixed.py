...
NEW_MIRROR_DEBUG_CHATS_RAW = os.environ.get(
    ...
    "-1003812195730,-1003371106919,-1003651353503,-1003087047858,-1002817163788,-1003770573057",
).strip()
...
# BEGIN FORCE_LAST21_2438454194_TO_5655_V1

async def force_last21_2438454194_to_5655_v1():
    """
    Copy the newest 21 real/copyable messages from
    -1002438454194 into private topic 5655 exactly once.

    Progress is persisted per source message so a partial startup
    interruption will not intentionally resend completed items.
    """

    route = next(
        (
            r for r in ROUTES
            if int(r.get("source_chat", 0)) == -1002438454194
            and r.get("source_topic") is None
            and int(r.get("dest_chat", 0)) == -1003852763875
            and int(r.get("dest_topic", 0)) == 5655
        ),
        None,
    )
    ...
    if route is None:
        raise RuntimeError(
            "2438454194 -> 5655 route not found"
        )

    done_file = DATA_DIR / "last21_2438454194_to_5655_v1.done"

    progress_file = (
        DATA_DIR
        / "last21_2438454194_to_5655_v1.progress.json"
    )

    if done_file.exists():

        log.warning(
            "[FORCE 2438454194 LAST21 V1 ALREADY DONE] "
            "dest=-1003852763875_5655"
        )

        return

    log.warning(
        "[FORCE 2438454194 LAST21 V1 BEGIN] "
        "source=-1002438454194 "
        "dest=-1003852763875_5655 "
        "requested=21 sender=KratosFX"
    )

    # Fetch substantially more than 21 so Telegram service /
    # unsupported messages do not reduce the requested real count.
    candidates = await get_route_poll_messages(
        route,
        100,
    )
    ...
    copyable = [
        ...
        if text_of(message) or is_real_media(message)
    ]
    ...
    selected = copyable[:21]

    if len(selected) < 21:

        raise RuntimeError(
            f"Only {len(selected)} copyable messages found; "
            "expected 21"
        )

    # Destination should read in normal chronological order.
    selected.sort(
        key=lambda message: int(
            getattr(message, "id", 0) or 0
        )
    ...
    log.warning(
        "[FORCE 2438454194 LAST21 V1 SELECTED] "
        f"ids={selected_ids}"
    )
    ...
    for message in selected:
        ...
        if mid in completed:
            ...
            log.warning(
                "[FORCE 2438454194 LAST21 V1 PROGRESS SKIP] "
                f"source_msg={mid}"
            )

            continue

        # Protect against a partial earlier attempt whose message-map
        # write succeeded before the progress file was written.
        try:

            mapped = existing_destination_ids(
                message,
                route,
            )
        ...
        if mapped:
            ...
            log.warning(
                "[FORCE 2438454194 LAST21 V1 ALREADY MAPPED] "
                f"source_msg={mid} dest_msgs={mapped}"
            )
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                log.warning(
                    "[FORCE 2438454194 LAST21 V1 COPIED] "
                    f"source_msg={mid} "
                    f"completed={len(completed)}/21 "
                    "dest=-1003852763875_5655"
                )
                ...
                log.warning(
                    "[FORCE 2438454194 LAST21 V1 FLOODWAIT] "
                    f"source_msg={mid} "
                    f"wait={wait_for}s"
                )
                ...
                log.exception(
                    "[FORCE 2438454194 LAST21 V1 ITEM FAILED] "
                    f"source_msg={mid} "
                    f"attempt={attempt}/3 "
                    f"{type(exc).__name__}: {exc}"
                )
        ...
        if not success:
            ...
            raise RuntimeError(
                f"Failed source message {mid}: {final_error}"
            )
    ...
    log.warning(
        "[FORCE 2438454194 LAST21 V1 DONE] "
        f"selected={selected_ids} "
        f"copied_now={copied_now} "
        "dest=-1003852763875_5655"
    )

# END FORCE_LAST21_2438454194_TO_5655_V1
...
async def main():
    ...
    await force_last21_2438454194_to_5655_v1()
    await verify_route_titles_once()
