...
# BEGIN FORCE_LAST20_2521699926_V7

async def force_last20_2521699926_to_5762_v7():
    """
    Copy exactly the latest 20 source messages into topic 5762 as KratosFX.
    Then normal live-only forwarding continues.

    A persistent marker prevents repeating the completed 20-message batch.
    Existing message mappings prevent duplicates after a partial failure.
    """

    route = next(
        (
            r for r in ROUTES
            if int(r.get("source_chat", 0)) == -1002521699926
            and int(r.get("dest_chat", 0)) == -1003852763875
            and int(r.get("dest_topic", 0)) == 5762
        ),
        None,
    )
    ...
    if route is None:
        raise RuntimeError(
            "Route -1002521699926 -> -1003852763875_5762 not found"
        )

    marker = DATA_DIR / "last20_2521699926_to_5762_v7.done"

    if marker.exists():
        log.warning(
            "[FORCE LAST20 ALREADY DONE] "
            "source=-1002521699926 dest=-1003852763875_5762"
        )
        return
    ...
    log.warning(
        ...
        "dest=-1003852763875_5762 "
        "requested=20 sender=KratosFX"
    )
    ...
    log.warning(
        f"[FORCE LAST20 FETCHED] count={len(messages)}"
    )

    if len(messages) == 0:
        raise RuntimeError(
            "Telegram returned zero messages from -1002521699926"
        )
    ...
    for message in messages:
        ...
        if existing_destination_ids(message, route):
            ...
            log.warning(
                f"[FORCE LAST20 ALREADY MAPPED] source_msg={mid}"
            )
            ...
            try:
                ...
                if sent is None:
                    if not text_of(message) and not is_real_media(message):
                        ...
                        log.warning(
                            f"[FORCE LAST20 UNSUPPORTED] source_msg={mid}"
                        )
                ...
                log.warning(
                    f"[FORCE LAST20 COPIED] "
                    f"source_msg={mid} "
                    f"number={copied} "
                    f"dest=-1003852763875_5762"
                )
                ...
                final_error = exc
                wait_seconds = min(int(exc.seconds) + 1, 120)

                log.warning(
                    f"[FORCE LAST20 FLOODWAIT] "
                    f"source_msg={mid} wait={wait_seconds}s"
                )
